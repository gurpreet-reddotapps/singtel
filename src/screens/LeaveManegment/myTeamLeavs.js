...
import { getApprovedRequest, getLeaveRequest, getRejectedRequest, getSubOrdinateRequest } from '../../api';
...
const MyTeamLeaves = ({ navigation, route }) => {
    const [email, setEmail] = useState("");
    const [activeTab, SetActiveTab] = useState("0");
    const [selectedCategory, SetselectedCategory] = useState(0);
    const [leavePendings, SetLeavePendings] = useState([]);
    const [leaveApprovals, SetLeaveApprovals] = useState([]);
    const [leaveRejected, SetLeaveRejected] = useState([]);
    ...
    function GetLeaves() {
        getApprovedRequest().then((res) => {
            SetLeaveApprovals(res.data),
                console.log("SDHJKSDJKSGDKJSGKJD", res?.data)
        })
        getRejectedRequest().then((res) => {
            SetLeaveRejected(res.data),
                console.log("SDHJKSDJKSGDKJSGKJD", res?.data)
        })
        getSubOrdinateRequest().then((res) => {
            SetLeavePendings(res.data),
                setLoading(false)
        })
    }
        ...
        return (
            <View style={{ flex: 1, paddingTop: 20 }} >
                <FlatList
                    ...
                    data={leaveApprovals?.leave_requests}
        ...
        return (
            <View style={{ flex: 1, paddingTop: 20 }} >
                <FlatList
                    ListEmptyComponent={ListEmptyComponent}
                    directionalLockEnabled style={{ alignSelf: "center", }}
                    data={leavePendings?.leave_requests}
                    renderItem={(item, index) => LeaveCardComponent({ data: item.item, index: item.index })} />
            </View>
        )
    }
        ...
        return (
            <View style={{ flex: 1, paddingTop: 20 }} >
                <FlatList
                    ListEmptyComponent={ListEmptyComponent}
                    directionalLockEnabled style={{ alignSelf: "center", }}
                    data={leaveRejected?.leave_requests}
                    renderItem={(item, index) => LeaveCardComponent({ data: item.item, index: item.index })} />
            </View>
        )
    }
    ...
    return (
        <View style={styles.container} >
            <CustomHeader backIcon title={"My Team's Leave"} />
            {/* <View style={{ flex: 1, paddingTop: 20 }} >
                <FlatList
                    ListEmptyComponent={ListEmptyComponent}
                    directionalLockEnabled style={{ alignSelf: "center", }}
