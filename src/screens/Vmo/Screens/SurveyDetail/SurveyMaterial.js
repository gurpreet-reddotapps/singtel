...
const SurveyMaterial = (props) => {
    ...
    const AmountWithid = async (amountVal) => {
        console.log("Material Item Remark in the center :-", amountVal);
        console.log("Material Item Remark in the center :-", props.index);
        setapprovedAmount(amountVal)
        if (amountVal !== null && amountVal !== NaN) {
            props.addApproveAmount(amountVal, props.index)
        } else {
            props.addApproveAmount(0, props.index)
        }
    }
        ...
        <>
            <Pressable style={styles.FuelCardAreaNNT}  >
                <ImageBackground
                    ...
                    style={[styles.UpperCardArea, { backgroundColor: '#FFA600' }]} >
                ...
                <View style={styles.CardWrapper} >
                            ...
                            <View style={styles.StartDate}>
                                <View style={styles.UpperText}>
                                    ...
                                    <Pressable onPress={() => alertOnPress()} >
                                        <TextInput placeholder='Enter Remark' editable={false} style={[styles.StatusText, { color: '#F0A500', }]} >{props?.item?.remark}</TextInput>
                                    </Pressable>
                                </View>
                            </View>
                        </View>
                    ...
                    <View style={styles.threeItem} >
                        ...
                        {props?.inputAmount == "Item by item" ?
                            <View style={{ width: '100%', paddingHorizontal: 20, }} >
                                <Text style={styles.TextColor} >Approval Amount</Text>
                                <TextInput
                                    style={styles.TextInputStyles}
                                    placeholder="Enter the number"
                                    onChangeText={text => AmountWithid(text)}
                                    autoCorrect={false}
                                    keyboardType="numeric"
                                    autoCapitalize="none"
                                // editable={false}
                                >
                                    {approvedAmount == undefined ? props?.item?.approved_cost : approvedAmount}
                                </TextInput>
                            </View>
                            : null}
                    </View>
