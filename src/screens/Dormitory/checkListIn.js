...
const CheckListIn = ({ navigation }) => {
    ...
    return (
        <View style={{ flex: 1, backgroundColor: 'white' }} >
            ...
            <View style={{ flex: 1 }} />
            <ButtonComponent onPress={()=>setShowSuccess(true)} style={{ width: width / 1.10, alignSelf: "center" }} title={"Confirm checkin"} bgColor={colors.primaryColor} />

            <ServiceModalCheckInOut  
            visible={showSuccess}
            onClose={()=>{setShowSuccess(false),navigation.navigate(routes.dormitory)}} 
            title="Items verified & check in successfully!    Next verification is on monday next week." />
        </View>
    )
}
